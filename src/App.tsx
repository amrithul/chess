...
const App = () => {
  ...
  const onlineBackendAvailable = Boolean(socketUrl);
  ...
  const playerTurnLabel = mode === 'online' && playerColor
    ? snapshot.turn === (playerColor === 'w' ? 'w' : 'b')
      ...
      : "Opponent's turn"
    : roomStatusLabel;
  const isPlayerTurn = mode === 'online'
    ? !playerColor || snapshot.turn === (playerColor === 'w' ? 'w' : 'b')
    : mode === 'ai'
      ? snapshot.turn === 'w'
      : true;
  ...
  }, [mode, pendingCreateRoom, settings.username, socketUrl]);
    ...
    <div className={`app-shell ${theme}`}>
      ...
      <AnimatePresence mode="wait">
        <Routes>
          ...
          <Route path="/play/local" element={<motion.main initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="board-layout"><div className="board-card"><Board fen={snapshot.fen} sideToMove={snapshot.turn} onMove={handleMove} theme={theme} boardTheme={boardTheme} showLegalMoves={settings.showLegalMoves} legalMoves={legalMoves} lastMove={snapshot.lastMove ? { from: snapshot.lastMove.slice(0, 2), to: snapshot.lastMove.slice(2, 4) } : null} orientation={orientation} isYourTurn={isPlayerTurn} /></div><aside className={`right-drawer ${drawerOpen ? 'open' : 'closed'}`}><Panel mode={mode} roomCode={roomCode} setRoomCode={setRoomCode} onlineRoom={onlineRoom} joinPromptOpen={joinPromptOpen} setJoinPromptOpen={setJoinPromptOpen} handleCreateRoom={handleCreateRoom} submitJoinRoom={submitJoinRoom} chatMessages={chatMessages} chatText={chatText} setChatText={setChatText} handleSendChat={handleSendChat} playerTurnLabel={playerTurnLabel} onlineBackendAvailable={onlineBackendAvailable} /></aside></motion.main>} />
