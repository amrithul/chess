...
const Board = ({
  ...
  const handleSquareClick = (square: string) => {
    ...
    if (selectedSquare) {
      ...
      if (matchingMoves.length) {
        const promotionMove =
          matchingMoves.find((legalMove) => legalMove.length === 5 && legalMove.endsWith('q')) ??
          matchingMoves.find((legalMove) => legalMove.length === 5);
        const promotion = promotionMove?.length === 5 ? promotionMove[4] : undefined;
        setSelectedSquare(null);
        onMove(from, to, promotion);
  ...
  return (
    <div className={`board-surface ${theme}`}>
      <div className={`board-grid ${boardTheme}`}>
        {orderedSquares.map((square) => {
          ...
          const isLegalTarget = Boolean(
            selectedSquare &&
              showLegalMoves &&
              legalMoves.some((move) => move.startsWith(selectedSquare) && move.slice(2, 4) === square)
          );
          const isCapture = Boolean(
            isLegalTarget &&
              piece &&
              selectedSquare &&
              getPieceColor(piece) !== getPieceColor(getPieceAt(selectedSquare))
          );

          // Highlight for king in check
          const isKingInCheck = isCheckPosition && piece?.toLowerCase() === 'k';
          ...
          return (
            ...
            >
              {piece ? (
                <span className={`piece piece-${getPieceColor(piece)}`}>
      ...
      {showCheckAlert && !isCheckmate && (
        <div className="check-overlay" role="status" aria-live="polite">
          <span>⚠ Check</span>
        </div>
      )}
      {isCheckmate && (
        <div className="check-overlay" role="status" aria-live="polite" style={{ background: 'rgba(34, 197, 94, 0.1)' }}>
          <span style={{ background: 'rgba(34, 197, 94, 0.92)' }}>🏆 Checkmate!</span>
        </div>
      )}
      {isDraw && (
        <div className="check-overlay" role="status" aria-live="polite" style={{ background: 'rgba(245, 158, 11, 0.1)' }}>
          <span style={{ background: 'rgba(245, 158, 11, 0.92)' }}>🤝 Draw</span>
        </div>
      )}
    </div>
  );
